...
BEGIN 
    OPEN c_file_size ;
        ...
        DBMS_OUTPUT.PUT_LINE('Tablespace Name = ' || v_table_name ||
                    ', Size (MB) = ' || v_size_file || 
                    ', Used (MB) = ' || v_used_bytes ||
                    ', Free (MB) = ' || v_free_bytes || 
                    ', Used % = ' || v_procent_used );
        msg_body := (v_size_file || v_used_bytes || v_free_bytes || v_procent_used);
            IF v_procent_used > 80 THEN 
                send_mail('my@post.test.com', ' Please check the size of the DBF files!', msg_body);
            ELSE 
                send_mail('my@post.test.com', ' Please check configuration!', msg_body);
            END IF;
        END LOOP;
    CLOSE c_file_size;
END;
